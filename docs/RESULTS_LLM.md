...
$n_{\text{trials}}=30$ rerun of the final certificate version (Tables
`tab:llmfull` and `tab:waterdrum` in `paper/main.tex`; raw data in
`results/llm_watermark_raw.csv`), which both narrows the confidence
intervals on every rate below and reveals that the 3-trial WaterDrum
comparison below was, by chance, more flattering to SEAL-W than the
30-trial data supports (a modest ~10% vs. ~7% detection gap, not a clean
per-trial win). The rest of this document is kept as-is: a development
log of the 3-trial run and the successive fixes that produced the
current certificate version, not a claim that these specific numbers
are the paper's final reported values.
