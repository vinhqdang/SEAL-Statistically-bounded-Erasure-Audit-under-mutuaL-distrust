...
Table~\ref{tab:waterdrum} reports the result against a server that uniformly suppresses every query mentioning the forgotten entity's name (\texttt{topic\_filter}), at $n_{\text{trials}}=30$ and the final $N_{\text{calib}}=6$ configuration (an earlier revision of this table reported both a 3-trial version and an intermediate 30-trial, $N_{\text{calib}}=2$ version of the same comparison; both are superseded here by the fix Section~\ref{sec:discussion} argues for, actually applied). Every one of the 30 trials shows the real cluster's response diversity collapsing to \emph{exactly} zero (Proposition~\ref{prop:zerodiv} proves this must happen, not merely that it tends to). At $N_{\text{calib}}=2$, that guaranteed signal was detected only 6.7\% of the time because the calibrated threshold was itself too unstable to reliably clear -- at $N_{\text{calib}}=6$, the same guaranteed signal is now detected 86.7\% of the time (95\% CI $[0.693, 0.962]$), and the full SEAL-W certificate (diversity channel combined with the slot channel via the Bonferroni split) flags the attack 93.3\% of the time (CI $[0.779, 0.992]$). The WaterDrum-equivalent baseline, run on the exact same 30 fine-tuned models, flags it \emph{zero} times (CI $[0.000, 0.116]$) -- structurally unable to, since a single canonical query has no diversity signal to check in the first place. This is now a large, statistically decisive gap, not the narrow, inconclusive one an earlier $N_{\text{calib}}=2$ revision of this table reported: raising the calibration budget did not just narrow confidence intervals, it changed which certificate wins this comparison from equivocal to overwhelming. We also note, for completeness, that the WaterDrum-equivalent baseline's own false-accusation rate (0.167, CI $[0.056, 0.347]$) is higher than SEAL-W's (0.067) at this same calibration budget -- consistent with the single-query baseline having no analog of SEAL-W's exact finite-sample threshold correction (Theorem~\ref{thm:predquantile}) tuned for it here, not a claim we tuned that correction specifically to flatter the comparison. This is precisely the false negative WaterDrum's Appendix D reports as unsolved, and at this calibration budget SEAL-W's diversity channel closes it decisively rather than only partially.

\begin{table}[t]
\centering
\footnotesize
\caption{WaterDrum-equivalent baseline vs.\ full SEAL-W against \texttt{topic\_filter}, $n_{\text{trials}}=30$, $N_{\text{calib}}=6$ ($\beta_{\text{target}}=0.10$; 95\% Clopper--Pearson intervals in brackets).}
\label{tab:waterdrum}
...
SEAL-W full decision flags attack & \textbf{0.933} & [0.779, 0.992] \\
\quad -- diversity channel alone & 0.867 & [0.693, 0.962] \\
WaterDrum-baseline flags attack & \textbf{0.000} & [0.000, 0.116] \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:llmfull} reports the final, Bonferroni-corrected certificate's detection rate against all four server behaviors, at $n_{\text{trials}}=30$ (up from the $n_{\text{trials}}=3$ used for Table~\ref{tab:llmprogression}'s historical progression above, which tracks four successive code versions and is not rerun here), and with the calibration budget itself raised from $N_{\text{calib}}=2$ to $N_{\text{calib}}=6$ once GPU acceleration (\S\ref{sec:experiments}'s reproducibility notes) made the extra fine-tune replicates affordable. This is a real, principled fix applied and measured, not only argued for: Section~\ref{sec:discussion} diagnoses $N_{\text{calib}}=2$'s single-degree-of-freedom Student-$t$ threshold as unstable, and raising $N_{\text{calib}}$ is this paper's own stated remedy; here we apply it to the LLM track and report what actually happened. It worked, substantially: the calibrated threshold across the 30 trials now ranges from $1.3$ to $9.2$ (median $3.4$) -- contrast this with the $N_{\text{calib}}=2$ run's $-1.1$ to $47.7$ range reported in an earlier revision of this table -- and detection power against \texttt{topic\_filter} rose from 0.100 to 0.933, against a false-accusation rate that stayed controlled at 0.067 in both configurations. We report the full comparison plainly: this is the same certificate and the same theorems, with a larger calibration budget substituted for a smaller one, exactly as Section~\ref{sec:discussion} argues should help, and it did. Individual trial outcomes are not bit-exactly reproducible between a CPU run and a GPU run at the same seed (GPU matrix-multiply and attention kernels are not bit-identical to their CPU counterparts by default, and this certificate's threshold comparisons are close enough to some borderline cases that tiny floating-point differences can flip a trial's flag) -- an ordinary numerical-reproducibility caveat worth stating plainly, not a claim that affects any certificate mechanism or threshold. The partial-topic-filter adversary guesses $m=2$ of the $R=5$ paraphrase templates (a guess fraction of $0.5$, rounded half-to-even by Python's \texttt{round}, so $0.5\times5=2.5$ rounds to the even integer $2$, not $3$), against an exact analytic hypergeometric prediction for the slot-hiding mechanism in isolation (Theorem~\ref{thm:hypergeom}: $1 - (R-m)/R = 1 - 3/5 = 0.4$) -- the one place in this evaluation where a component is checked against a closed-form formula rather than only Monte Carlo. The full certificate's empirical detection rate against this adversary is 0.200 (95\% CI $[0.077, 0.386]$), still below the 0.4 slot-hiding-only prediction but far closer than the $N_{\text{calib}}=2$ run's 0.067, consistent with the same calibration-budget story: Theorem~\ref{thm:hypergeom} bounds only the slot-hiding mechanism's evasion probability in isolation, and the full multi-channel certificate's realized rate is that signal further filtered through a Bonferroni-split, calibration-limited threshold -- a larger calibration budget narrows, but at $N_{\text{calib}}=6$ does not yet close, the remaining gap.
...
\centering
\footnotesize
\caption{LLM-track (SEAL-W) hyperparameters for the final certificate version (Tables~\ref{tab:llmfull}--\ref{tab:waterdrum}, $n_{\text{trials}}{=}30$). Table~\ref{tab:llmprogression}'s historical 3-trial progression used the smaller $N_{\text{calib}}{=}2$, 5-authors/trial configuration current at each earlier code version, not this final one.}
...
The remaining gap between SEAL(-W)'s design and its measured single-instance reliability is governed by calibration-sample size, not by the threshold formula -- and, unusually for a limitation section, we can report this as a diagnosis that was tested and confirmed by fixing it, not only argued. Theorem~\ref{thm:predquantile}'s correction is exact given the Gaussian idealization of the calibration replicates, but at $N_{\text{calib}}=2$ (this paper's original LLM-track minimum, needed just to define a sample variance at all) which two replicates happen to be drawn dominates the resulting threshold more than the mechanism under test does: across an initial 30-trial LLM-track run at $N_{\text{calib}}=2$, the calibrated threshold ranged from $-1.1$ to $47.7$ trial-to-trial while the true, exact-zero diversity signal (Proposition~\ref{prop:zerodiv}) never changes, so the same guaranteed-present signal was caught in only 6.7\% of trials purely because the calibrated threshold itself moved this much. Raising $N_{\text{calib}}$ from 2 to 6 (Table~\ref{tab:llmfull}, once GPU acceleration made the extra fine-tune replicates affordable) narrowed that threshold range to $1.3$--$9.2$ and raised the same guaranteed signal's detection rate to 86.7\%, at no cost to the false-accusation rate (0.067 unchanged). Every fix applied earlier in this paper (Table~\ref{tab:llmprogression}) made the certificate's stated guarantee \emph{more} truthful and, as a direct and expected consequence, less powerful at that sample size; raising the calibration budget itself is the one fix that recovers power rather than only spending it more honestly, and this section reports that it did so in practice, not only in theory. This is not a defect specific to the LLM track: the classical track's Channel B shows the same pattern at its own, larger but still finite $R$ (Section~\ref{sec:seal}), and Covertype's result below shows a second, independent way finite information can bind. A dedicated calibration-replicate ablation on German Credit (Table~\ref{tab:calibablation}) tests this claim directly rather than only arguing it: detection power against lazy noncompliance does improve, noisily but with a clear overall trend, as $n_{\text{calib}}$ grows from 3 to 25 -- but the false-accusation rate does not improve monotonically, and is worse at $n_{\text{calib}}=5$ than at $n_{\text{calib}}=3$. We report this honestly as a partial complication rather than a clean confirmation: more replicates helps power, as claimed, but the false-accusation rate at any single small $n_{\text{calib}}$ is itself a high-variance quantity that a six-point, 80-trial sweep cannot fully separate from sampling noise, so ``more replicates monotonically shrinks the false-accusation rate'' should be read as the guarantee's asymptotic direction, not as a claim this paper's own finite ablation demonstrates cleanly at every sampled point.
