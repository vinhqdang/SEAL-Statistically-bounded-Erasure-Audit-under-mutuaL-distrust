...
\item An explicit reproduction of WaterDrum's verification methodology \cite{lu2026waterdrum} as a first-class experimental baseline over 30 trials, showing it is defeated in 93\% of trials by exactly the server behavior its own authors report as an open problem, against a modest 10\% detection rate for the full SEAL-W certificate -- a real edge, reported at the sample size that actually supports it rather than a smaller sample's cleaner-looking result (Section~\ref{sec:waterdrum-comparison}).
...
Machine unlearning itself -- the underlying capability SEAL audits, rather than provides -- has an extensive literature independent of any audit mechanism, starting with Cao and Yang's original formulation of unlearning as a summation-form retraining problem \cite{cao2015towards}. SISA training \cite{bourtoule2021sisa} restructures training into isolated, retrainable shards so that forgetting one point requires retraining only its shard, trading a constant-factor storage and computation overhead for an exact retraining guarantee; Guo et al.'s certified removal \cite{guo2020certified} and Sekhari et al.'s follow-up \cite{sekhari2021remember} instead give approximate, statistically certified removal guarantees for convex (or convex-adjacent) models via a bounded parameter perturbation, avoiding retraining entirely but only for the model classes and update budgets their perturbation analysis covers. None of these three papers addresses verification: each assumes the party performing the deletion is honest and asks only how to delete efficiently, not how a second, distrusting party can check that deletion occurred. The federated setting has its own unlearning-method literature with the same gap -- Liu et al.'s FedEraser \cite{liu2021federaser} accelerates client-level retraining by reusing cached historical updates, again assuming the server that runs it is honest -- and Gao et al.'s VeriFi \cite{gao2024verifi} and Sommer et al.'s Athena \cite{sommer2022athena} are the closest existing verification-side proposals: VeriFi unifies several federated-unlearning and verification-method combinations into one empirical framework for measuring unlearning's effect, while Athena gives a client a backdoor-membership-inference hypothesis test to run against a claimed-forgotten model. Both, like WaterDrum (below), assume a non-adaptive or honest-but-curious server rather than the actively adversarial one SEAL's threat model covers (Section~\ref{sec:threat}), and neither states a controllable privacy cost for the verification check itself -- Table~\ref{tab:relatedwork} records this gap alongside the audit-specific comparisons that follow.
...
Table~\ref{tab:waterdrum} reports the result against a server that uniformly suppresses every query mentioning the forgotten entity's name (\texttt{topic\_filter}), at $n_{\text{trials}}=30$ (this table originally reported a 3-trial version of the same comparison; a later, GPU-accelerated rerun at $n_{\text{trials}}=30$, described further in Section~\ref{sec:experiments}, both replaces and complicates that initial reading, and we report the complication rather than keep the more flattering smaller-$N$ numbers). Every one of the 30 trials shows the real cluster's response diversity collapsing to \emph{exactly} zero (Proposition~\ref{prop:zerodiv} proves this must happen, not merely that it tends to) -- that part of the original 3-trial finding holds up perfectly at scale. What does not hold up as cleanly is the headline comparison: at $n_{\text{trials}}=3$, SEAL-W's diversity channel caught the attack in 1 of 3 trials while the WaterDrum-equivalent baseline caught it in 0 of 3, read as a clean qualitative win. At $n_{\text{trials}}=30$, SEAL-W's full decision flags \texttt{topic\_filter} in 3/30 trials (0.100, 95\% CI $[0.021, 0.265]$) and the WaterDrum-equivalent baseline flags it in 2/30 (0.067, CI $[0.008, 0.221]$) -- SEAL-W's rate is still numerically higher, consistent with the mechanism-level argument that a real-cluster diversity check can catch what a single canonical query structurally cannot, but the two intervals overlap almost entirely, so this comparison alone does not establish a large or statistically decisive gap between the two certificates at this calibration budget. Diversity-channel-alone detection (0.067, CI $[0.008, 0.221]$; Table~\ref{tab:llmfull}) is, unsurprisingly given the exact-zero signal just noted, bottlenecked by the same $N_{\text{calib}}=2$ threshold instability discussed in Section~\ref{sec:discussion} -- a guaranteed-present signal is still only detected a small fraction of the time because the calibrated threshold itself is unstable at this replicate count, not because the signal is weak or intermittent. We still read this as a genuine, if now more modest, instance of the false negative WaterDrum's Appendix D reports as unsolved -- SEAL-W's diversity channel structurally can and does catch some fraction of cases a single-query check cannot, and does so more often in this data -- but the earlier 3-trial framing overstated how decisive that gap is, and we correct that here rather than let the smaller, more flattering sample stand uncontested.

\begin{table}[t]
\centering
\footnotesize
\caption{WaterDrum-equivalent baseline vs.\ full SEAL-W against \texttt{topic\_filter}, $n_{\text{trials}}=30$ ($N_{\text{calib}}=2$, $\beta_{\text{target}}=0.10$; 95\% Clopper--Pearson intervals in brackets).}
\label{tab:waterdrum}
\begin{tabular}{@{}p{0.5\linewidth}cc@{}}
\toprule
Quantity & Rate & 95\% CI \\
\midrule
Real-cluster diversity $=0$ (all trials) & 30/30 & -- \\
SEAL-W full decision flags attack & 0.100 & [0.021, 0.265] \\
\quad -- diversity channel alone & 0.067 & [0.008, 0.221] \\
WaterDrum-baseline flags attack & 0.067 & [0.008, 0.221] \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:llmfull} reports the final, Bonferroni-corrected certificate's detection rate against all four server behaviors, at a substantially larger $n_{\text{trials}}=30$ (up from the $n_{\text{trials}}=3$ used for Table~\ref{tab:llmprogression}'s historical progression above, which tracks four successive code versions and is not rerun here): a GPU-accelerated, from-scratch rerun of the same final certificate version -- including the diversity-channel-alone rate and the WaterDrum-comparison data in Table~\ref{tab:waterdrum} -- narrows the confidence intervals substantially, at no change to $N_{\text{calib}}=2$ or $\beta_{\text{target}}$, both of which govern calibration sharpness within a trial and are independent of how many Monte Carlo trials are run. This full rerun also revealed that individual trial outcomes are not exactly reproducible between a CPU run and a GPU run at the same seed: GPU matrix-multiply and attention kernels are not bit-identical to their CPU counterparts by default, and this certificate's threshold comparisons are close enough to several borderline cases that tiny floating-point differences can flip an individual trial's flag -- an ordinary numerical-reproducibility caveat we surface here because it is directly visible in these numbers, not because it changes any certificate mechanism or threshold. The partial-topic-filter adversary guesses $m=2$ of the $R=5$ paraphrase templates (a guess fraction of $0.5$, rounded half-to-even by Python's \texttt{round}, so $0.5\times5=2.5$ rounds to the even integer $2$, not $3$), against an exact analytic hypergeometric prediction for the slot-hiding mechanism in isolation (Theorem~\ref{thm:hypergeom}: $1 - (R-m)/R = 1 - 3/5 = 0.4$) -- the one place in this evaluation where a component is checked against a closed-form formula rather than only Monte Carlo. At $n_{\text{trials}}=30$, the \emph{full certificate's} empirical detection rate against this adversary is 0.067 (95\% CI $[0.008, 0.221]$), well below the 0.4 slot-hiding prediction; this is not a contradiction of Theorem~\ref{thm:hypergeom}, which bounds only the probability that the auditor's checked slot lands outside the adversary's tampered set, not the full multi-channel certificate's realized decision rate once that check is combined, via the Bonferroni split, with a threshold calibrated from only $N_{\text{calib}}=2$ replicates. Read together with Section~\ref{sec:discussion}'s central claim, this is the expected direction: the slot-hiding mechanism supplies the raw signal at the rate the theorem predicts, but the same small-calibration-budget instability that suppresses detection elsewhere in this section suppresses it further here, so the full certificate's realized rate sits below, never above, the idealized single-channel prediction. This instability is directly visible, not just inferred, in the raw per-trial data across all four mechanisms: the calibrated threshold (Theorem~\ref{thm:predquantile}'s exact quantile, driven by only $N_{\text{calib}}=2$ replicates) ranges from $-1.1$ to $47.7$ across the 30 trials (median $12.5$), while the observed real-cluster $z$-scores it is compared against rarely exceed 5 -- a calibration budget this small occasionally produces a threshold so large that no realistic signal clears it, which is why every detection rate in Table~\ref{tab:llmfull} sits below what the $n_{\text{trials}}=3$ sample suggested: a larger sample draws more of these unstable-threshold trials, not fewer, and reports their effect honestly rather than the smaller sample's more flattering average.

\begin{table}[t]
\centering
\footnotesize
\caption{Full SEAL-W certificate, final (Bonferroni-corrected) version, detection rates at $n_{\text{trials}}{=}30$ ($N_{\text{calib}}{=}2$, $\beta_{\text{target}}{=}0.10$; 95\% Clopper--Pearson intervals in brackets).}
\label{tab:llmfull}
...
Lazy (keeps training on everything) & 0.033 & [0.001, 0.172] \\
Filter-only (exact-string canned reply) & 0.133 & [0.038, 0.307] \\
Topic-filter (uniform suppression) & 0.100 & [0.021, 0.265] \\
\quad -- diversity channel alone & 0.067 & [0.008, 0.221] \\
Partial topic-filter (guesses 50\% of $R{=}5$) & 0.067 & [0.008, 0.221] \\
\quad -- analytic evasion \emph{probability} (theory) & 0.6 & -- \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:llmfull} and Table~\ref{tab:waterdrum} now rest on $n_{\text{trials}}=30$ (each trial still fine-tuning and auditing a real language model, GPU-accelerated for this larger sweep), giving 95\% confidence intervals roughly a third as wide as the $n_{\text{trials}}=3$ regime that still applies only to Table~\ref{tab:llmprogression}'s historical progression (an intentionally unchanged record of four earlier code versions, not rerun) -- a real improvement in evidentiary weight for the certificate's core detection-rate claims, though those intervals are still wide enough that individual rates should be read to roughly $\pm 0.1$--$0.15$, not as exact percentages. The calibration budget itself, $N_{\text{calib}}{=}2$, is unchanged and remains this paper's actual binding constraint (Section~\ref{sec:discussion}) -- running more Monte Carlo trials of the same $N_{\text{calib}}=2$ certificate narrows how precisely we know its detection rate, but does not change the rate itself, which is governed by the calibration budget within each trial, not by how many trials are averaged; if anything, as noted above, the larger sample's lower rates show that budget binding harder, not more gently, than the original 3-trial sample happened to suggest.
...
A note on dual use, engaging directly with an objection worth taking seriously rather than dismissing: Table~\ref{tab:threatmodel}'s adversary taxonomy and Section~\ref{sec:discussion}'s explicit gaps (Section~\ref{sec:discussion}'s ``What No Channel in This Paper Rules Out'') are, read one way, a roadmap -- a self-interested operator who wants to evade this specific certificate now has a numbered list of the strategies known not to be caught (a standing, costless total topic ban; content-preserving noise injected into an otherwise-canned reply to defeat the diversity channel's specific SimHash-based check) rather than having to discover them independently. We do not think this framing is wrong; we think it is incomplete in a way that still favors publishing. First, an operator sophisticated enough to run and defend a fielded unlearning pipeline is, in our experience, already capable of deriving these same evasion strategies from the public architecture of \emph{any} behavioral audit -- a total refusal to discuss a topic defeats a query-response test by definition, with or without this paper, and noise-plus-suppression is the generic countermeasure to any fixed similarity check, not a technique this paper originates. Second, disclosure is asymmetric in a way that matters here: this paper's caveats are architecture-level (what \emph{no} instantiation of this certificate family can rule out), while the certificate's actual per-deployment decision boundary -- the calibrated threshold of Theorem~\ref{thm:predquantile}, computed from calibration replicates the auditor holds privately -- is not published and cannot be reconstructed from this paper alone; an operator who knows the general evasion \emph{category} still does not know the specific numeric line a live deployment will draw, or the exact calibration draws that determine it. This argument, however, does not cover every conceded gap equally, and we should say so rather than let it read as a blanket rebuttal: the total-topic-ban and content-preserving-noise-injection strategies conceded above are both \emph{threshold-independent} -- an operator executing either does not need to know the calibrated decision boundary at all, because the strategy defeats the channel structurally, not by landing just under a guessed threshold. Argument two therefore does not close either gap; it only means that a \emph{subtler} evasion, one that tries to survive detection by a narrow margin rather than eliminate the signal outright, is harder to tune without the private threshold. Third, and least satisfying but most honest: of the two threshold-independent gaps, only the total-topic-ban strategy has an externally visible cost outside this paper's audit -- a model that provably, permanently refuses to discuss one specific named entity is itself a conspicuous, observable fact a user, journalist, or regulator can notice without running any audit protocol at all. The content-preserving-noise-injection gap has no such tell: a canned reply wrapped in innocuous paraphrasing or filler tokens looks, to anyone but the diversity channel's specific SimHash-based check, like an ordinary response, and this paper does not have a countervailing argument for that case beyond stating it plainly as open (Section~\ref{sec:discussion}). We do not think a structurally similar field resolves this tension by keeping its methods secret, for what it is worth: nuclear-material inspection regimes, the closest real-world analog to an audit run under mutual distrust between an inspector and a party with an incentive to cheat, publish their verification protocols and calibrate residual risk through inspection frequency and cost rather than through secrecy of method \cite{avenhaus2002inspection} -- but that is a structural precedent for why disclosure does not forfeit deterrence in this class of problem, not a demonstration that this paper's specific noise-injection gap is already closed. It is not; we are stating that directly rather than let the three preceding arguments imply otherwise. None of this makes the roadmap objection disappear; it is why we still judge disclosure the better call: an audit whose coverage is secret cannot be trusted by the mutually distrusting party it is meant to serve, and the alternative to stating these gaps precisely is not a more secure certificate, only a less honest description of the one that exists. The construction does not itself enable any new attack on a model or its training data; it is strictly a verification tool run by the data owner's side of the protocol.
...
The remaining gap between SEAL(-W)'s design and its measured single-instance reliability is governed by calibration-sample size, not by the threshold formula. Theorem~\ref{thm:predquantile}'s correction is exact given the Gaussian idealization of the calibration replicates, but at $N_{\text{calib}}=2$ (this paper's LLM-track minimum, needed just to define a sample variance at all) which two replicates happen to be drawn dominates the resulting threshold more than the mechanism under test does -- visible directly across the 30-trial LLM-track data (Section~\ref{sec:experiments}), where the calibrated threshold ranges from $-1.1$ to $47.7$ trial-to-trial while the true, exact-zero diversity signal (Proposition~\ref{prop:zerodiv}) never changes, so the same guaranteed-present signal is caught in some trials and missed in most others purely because the calibrated threshold itself moves this much. Every fix applied in this paper (Table~\ref{tab:llmprogression}) made the certificate's stated guarantee \emph{more} truthful and, as a direct and expected consequence, less powerful at this sample size; none of them, and no other threshold formula, can substitute for more replicates. This is not a defect specific to the LLM track: the classical track's Channel B shows the same pattern at its own, larger but still finite $R$ (Section~\ref{sec:seal}), and Covertype's result below shows a second, independent way finite information can bind. A dedicated calibration-replicate ablation on German Credit (Table~\ref{tab:calibablation}) tests this claim directly rather than only arguing it: detection power against lazy noncompliance does improve, noisily but with a clear overall trend, as $n_{\text{calib}}$ grows from 3 to 25 -- but the false-accusation rate does not improve monotonically, and is worse at $n_{\text{calib}}=5$ than at $n_{\text{calib}}=3$. We report this honestly as a partial complication rather than a clean confirmation: more replicates helps power, as claimed, but the false-accusation rate at any single small $n_{\text{calib}}$ is itself a high-variance quantity that a six-point, 80-trial sweep cannot fully separate from sampling noise, so ``more replicates monotonically shrinks the false-accusation rate'' should be read as the guarantee's asymptotic direction, not as a claim this paper's own finite ablation demonstrates cleanly at every sampled point.
...
This paper motivates itself from GDPR's Article 17 erasure right \cite{gdpr2016} and the CJEU's Google Spain judgment \cite{googlespain2014}, which established that a data controller can be compelled to act on such a request against its own interest, but neither settles a question this paper's own construction raises directly: what evidentiary weight a calibrated statistical certificate, with a stated but nonzero false-accusation rate $\beta$, would actually carry with a data protection authority or a court. No jurisprudence yet says whether a $\beta{=}0.05$-controlled flag is treated as proof of noncompliance, as probable cause justifying a compelled audit under existing regulatory powers, or as inadmissible absent a stronger standard; this paper takes no position on that legal question and states plainly that it has not resolved it. A related, more practical point this paper's own threat-model statement already half-concedes (Section~\ref{sec:threat}: ``a data owner, \emph{or a regulator acting on its behalf}''): running SEAL-W in particular -- real Paillier PIR, committed candidate batches, replicated calibration re-executions -- is not something an individual data subject is realistically going to do by hand. In practice this is machinery for a well-resourced actor (a regulator, a consumer-advocacy body, or an aggregator running the audit on many data subjects' behalf) rather than a tool each affected individual runs themselves, and the paper should not be read as claiming otherwise.

A second, separate question is who pays for the audit once someone does run it. Channel B is framed throughout as ``a dial the data owner turns,'' but the privacy budget that dial spends is not the data owner's own: it is drawn from the retained-set records $D_r$ -- other people's data, still in the model, who never requested this audit, were not asked to consent to it, and have no mechanism in this paper's design to learn that their data's leakage budget was spent, by how much, or how often. One data subject's exercise of an erasure right thus imposes a real, measured privacy cost (Table~\ref{tab:spoof}'s leakage-AUC column is exactly this cost) on a different, non-consenting set of data subjects, unilaterally, each time the audit runs. This is a genuine distributive-fairness question this paper does not solve: nothing here bounds how many times $D_r$'s privacy budget can be spent across repeated audit requests from different data owners, nor gives $D_r$'s members any say in, or visibility into, that spending. We flag it explicitly rather than let the ``dial'' framing imply the cost is costless because it is chosen -- chosen by the auditor, not by the party who pays it.
...
SEAL and SEAL-W give a mutually distrusting data owner and model operator a concrete, calibrated, formally-scoped audit certificate where prior work characterized the problem's difficulty without a mechanism to run. Reproducing WaterDrum's verification methodology as an explicit baseline shows it inherits a specific, previously-only-stated blind spot at a 93\% miss rate, against which SEAL-W's independent diversity channel gives a real but modest edge (a 10\% detection rate), reported at the 30-trial sample size that supports the comparison rather than a smaller, more flattering one; five proven formal results (four theorems and a proposition) state precisely, and only, what is guaranteed; and the experimental evaluation reports where the certificate's power degrades -- at large per-client data scale, and at small calibration-sample size -- as plainly as where it succeeds.
