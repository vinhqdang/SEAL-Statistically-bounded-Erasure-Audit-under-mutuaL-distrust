...
Table~\ref{tab:llmfull} reports the final, Bonferroni-corrected certificate's detection rate against all four server behaviors, at a substantially larger $n_{\text{trials}}=30$ (up from the $n_{\text{trials}}=3$ used for Table~\ref{tab:llmprogression}'s historical progression above, which tracks four successive code versions and is not rerun here): a GPU-accelerated re-run of the same final certificate version narrows the confidence intervals on the main detection-rate row substantially, at no change to $N_{\text{calib}}=2$ or $\beta_{\text{target}}$, both of which govern calibration sharpness within a trial and are independent of how many Monte Carlo trials are run. The diversity-channel-alone sub-row and the WaterDrum-comparison result (Section~\ref{sec:waterdrum-comparison}, Table~\ref{tab:waterdrum}) are not part of this larger re-run and remain at the original $n_{\text{trials}}=3$: infrastructure instability in the remote GPU environment used for the scale-up (repeated session disconnects immediately after the compute finished, before the fuller per-channel and WaterDrum-baseline data could be retrieved) meant only the five main per-mechanism flags -- which were also captured directly in this run's execution log, independent of the remote filesystem -- survived at the larger scale; recovering the rest requires a further rerun, left for that dedicated pass rather than blocking on it here. The partial-topic-filter adversary guesses $m=2$ of the $R=5$ paraphrase templates (a guess fraction of $0.5$, rounded half-to-even by Python's \texttt{round}, so $0.5\times5=2.5$ rounds to the even integer $2$, not $3$), against an exact analytic hypergeometric prediction (Theorem~\ref{thm:hypergeom}: $1 - (R-m)/R = 1 - 3/5 = 0.4$) -- the one place in this evaluation where a component is checked against a closed-form formula rather than only Monte Carlo. At $n_{\text{trials}}=30$, the empirical detection rate (0.200, 95\% CI $[0.077, 0.386]$) sits below the analytic 0.4, with the interval's upper edge only narrowly excluding it; the earlier $n_{\text{trials}}=3$ run's rate (0.333) was numerically closer but backed by a far wider interval that could not have distinguished the two possibilities. We report this as a real, if minor, discrepancy worth flagging rather than rounding it into agreement: it could reflect ordinary sampling variation still not fully resolved at $n=30$, or a genuine second-order effect the hypergeometric model's idealized assumptions (e.g.\ that the $m$ guessed templates are fixed independently of which one the auditor later checks) do not capture exactly in the real committed-batch implementation.

\begin{table}[t]
\centering
\footnotesize
\caption{Full SEAL-W certificate, final (Bonferroni-corrected) version, main detection rates at $n_{\text{trials}}{=}30$ ($N_{\text{calib}}{=}2$, $\beta_{\text{target}}{=}0.10$; 95\% Clopper--Pearson intervals in brackets). The diversity-channel-alone and evasion-probability rows are not part of this larger run -- see prose.}
\label{tab:llmfull}
...
Lazy (keeps training on everything) & 0.200 & [0.077, 0.386] \\
Filter-only (exact-string canned reply) & 0.233 & [0.099, 0.423] \\
Topic-filter (uniform suppression) & 0.333 & [0.173, 0.528] \\
\quad -- diversity channel alone ($n{=}3$) & 0.333 & -- \\
...
Table~\ref{tab:llmfull}'s main row now rests on $n_{\text{trials}}=30$ (each trial still fine-tuning and auditing a real language model, GPU-accelerated for this larger sweep), giving 95\% confidence intervals roughly a third as wide as the $n_{\text{trials}}=3$ regime everywhere else in this section (Table~\ref{tab:llmprogression}, the diversity-channel-alone and WaterDrum-comparison figures) -- a real improvement in evidentiary weight for the certificate's core detection-rate claim, though those intervals are still wide enough that individual rates should be read to roughly $\pm 0.1$--$0.15$, not as exact percentages. The calibration budget itself, $N_{\text{calib}}{=}2$, is unchanged and remains this paper's actual binding constraint (Section~\ref{sec:discussion}) -- running more Monte Carlo trials of the same $N_{\text{calib}}=2$ certificate narrows how precisely we know its detection rate, but does not change the rate itself, which is governed by the calibration budget within each trial, not by how many trials are averaged.
...
SEAL and SEAL-W give a mutually distrusting data owner and model operator a concrete, calibrated, formally-scoped audit certificate where prior work characterized the problem's difficulty without a mechanism to run. Reproducing WaterDrum's verification methodology as an explicit baseline shows it inherits a specific, previously-only-stated blind spot that SEAL-W's independent diversity channel closes; five proven formal results (four theorems and a proposition) state precisely, and only, what is guaranteed; and the experimental evaluation reports where the certificate's power degrades -- at large per-client data scale, and at small calibration-sample size -- as plainly as where it succeeds.
