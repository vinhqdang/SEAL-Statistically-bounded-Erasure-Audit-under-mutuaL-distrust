...
**Update:** the paper's main text now reports a larger, GPU-accelerated
...
false-accusation rate (0.067 unchanged) -- the calibration-budget fix
this document's own analysis below argues for, applied and confirmed.
An intermediate $n_{\text{trials}}=30$, $N_{\text{calib}}=2$ rerun
(kept only in git history, not restated here) briefly showed a much
narrower, statistically inconclusive WaterDrum-comparison gap before
this fix was applied; that intermediate result is superseded. The rest
of this document is kept as-is: a development log of the original
3-trial, $N_{\text{calib}}=2$ run and the successive fixes that
produced the current certificate version, not a claim that these
specific numbers are the paper's final reported values.
