...
The audited fictitious-author profiles can be built two ways: the default experiment invents author names and generates each author's watermarked training documents from scratch, or, additively, the same protocol can be grounded in the real, published TOFU benchmark \cite{maini2024tofu}. TOFU's ``full'' split lays out 200 fictitious authors as 4{,}000 QA rows in 200 contiguous 20-row blocks with no explicit author-id column, so names and facts must first be recovered from the raw text via two deterministic heuristics, hand-verified against three known authors and swept across all 200 blocks: an author's name is the most frequent two-word run of consecutive capitalized words in the block (possessive-normalized so ``X'' and ``X's'' reinforce one tally); a fact snippet is the first sentence matching a fixed keyword priority (genre, award, profession, notable work, birth date). Because this is a plurality vote, not verified extraction, we measured its confidence margin across all 200 blocks: typically unambiguous (median margin 27 over the runner-up), but not always -- 12 of 200 ($6\%$) end in an exact tie, and 30 of 200 ($15\%$) have a margin of 2 or fewer. We report this honestly rather than only the three-author spot-check: a non-trivial fraction of the pool may seed generation with a wrong or ambiguous name, though this affects only which name/fact seeds a synthetic training document, not any certificate mechanism, threshold, or theorem.

This does \emph{not} mean training on TOFU's literal QA text, which is static and carries no watermark bias -- using it directly would silently defeat the certificate. Instead, the recovered name/fact only seed the same, unmodified watermarked-generation call (\cite{kirchenbauer2023watermark}, Section~\ref{sec:sealw}) the default experiment uses with an invented name; every training document is still our own watermarked generation. The negative-canary pool is likewise drawn from the same real TOFU distribution and held out of all fine-tuning, for the same reason. This substitution changes only where the author pool's content comes from, not the certificate or any threshold in Sections~\ref{sec:seal}--\ref{sec:theory}; it is not comparable to the official TOFU leaderboard, which evaluates phi-1.5/LLaMA-2-7B against TOFU's own forget-quality metrics, not \texttt{distilgpt2} against a watermark/PIR certificate.
...
The remaining gap between SEAL(-W)'s design and its measured single-instance reliability is governed by calibration-sample size, not the threshold formula -- and, unusually for a limitation section, we report this as a diagnosis tested and confirmed by fixing it, not only argued. Theorem~\ref{thm:predquantile}'s correction is exact given the Gaussian idealization of the calibration replicates, but at $N_{\text{calib}}=2$ (this paper's original LLM-track minimum) which two replicates happen to be drawn dominates the resulting threshold more than the mechanism under test does: across an initial 30-trial run at $N_{\text{calib}}=2$, the calibrated threshold ranged from $-1.1$ to $47.7$ trial-to-trial while the true, exact-zero diversity signal (Proposition~\ref{prop:zerodiv}) never changes, so the same guaranteed-present signal was caught in only 6.7\% of trials purely because the threshold itself moved this much. Raising $N_{\text{calib}}$ from 2 to 6 (Table~\ref{tab:llmfull}, once GPU acceleration made the extra fine-tune replicates affordable) narrowed that range to $1.3$--$9.2$ and raised the same signal's detection rate to 86.7\%, at no cost to the false-accusation rate (0.067 unchanged). Every fix applied earlier in this paper (Table~\ref{tab:llmprogression}) made the certificate's stated guarantee \emph{more} truthful and, correspondingly, less powerful at that sample size; raising the calibration budget is the one fix that recovers power rather than only spending it more honestly, and this section reports that it did so in practice, not only in theory. This is not LLM-track-specific: the classical track's Channel B shows the same pattern at its own, larger but finite $R$ (Section~\ref{sec:seal}), and Covertype's result below shows a second, independent way finite information can bind. A dedicated calibration-replicate ablation on German Credit (Table~\ref{tab:calibablation}) tests this directly: detection power against lazy noncompliance improves, noisily but with a clear trend, as $n_{\text{calib}}$ grows from 3 to 25 -- but the false-accusation rate does not improve monotonically, worse at $n_{\text{calib}}=5$ than at $3$. We report this as a partial complication, not a clean confirmation: more replicates helps power, but the false-accusation rate at any single small $n_{\text{calib}}$ is itself high-variance enough that an 80-trial sweep cannot fully separate it from sampling noise, so ``more replicates shrinks the false-accusation rate'' should be read as the guarantee's asymptotic direction, not as cleanly demonstrated at every sampled point. One honest gap in this diagnosis: unlike German Credit's six-point sweep, the LLM track's $N_{\text{calib}}$ was tested at only two values, 2 and 6, chosen by what GPU compute made affordable at each point in this project's own timeline rather than by a target power or CI-width calculation; we have not swept $N_{\text{calib}}\in\{3,4,5,7,8\}$ for the LLM track, so we cannot rule out that some intermediate value performs worse than either endpoint, nor claim 6 is a minimal sufficient budget rather than simply the value that was tried and worked. This is exactly the calibration-budget-versus-power tradeoff Section~\ref{sec:futurework} poses as an open question rather than a solved one; readers should weigh the LLM-track result accordingly, as evidence that raising the budget can fix this failure mode, not as a demonstration of the smallest budget that does.

\subsubsection{Scale Is Not a Free Win}

Covertype's power loss is a genuine, dataset-scale-dependent limitation of the own-data channel, reported as such rather than tuned away. The mechanism is intuitive in retrospect but was not assumed going in: a federation with $\sim$200$\times$ more rows per client than German Credit converges to a substantially more stable optimum, so continuing to train on everything (the ``lazy'' adversary) perturbs the model's fit on any one client's forgotten records much less, relative to ordinary round-to-round training noise, than it does with a few hundred rows per client. Channel A's signal -- ``did removing this data change how well the model fits it'' -- is intrinsically weakest exactly where the model has the most other data to fall back on. Whether a tunable-cost channel analogous to Channel B, or a larger calibration budget, restores power at this scale is left as a direct next experiment.

\subsubsection{What No Channel in This Paper Rules Out}

Neither SEAL-W channel defeats a hypothetical server with a perfect, costless semantic classifier willing to refuse \emph{all} queries -- ordinary and audit alike -- about an entity, behaviorally indistinguishable from a total, standing topic ban rather than a targeted, audit-specific spoof. No black-box behavioral test, PIR-hidden or not, can formally rule this out: if $M'$ is willing to refuse to discuss an entity under any circumstance forever, no query-only audit can distinguish ``never learned about this entity'' from ``knows about it and refuses to ever say so.'' This is the same scope boundary Tang et al.'s impossibility framing draws for behavioral audits generally \cite{tang2026geometric}, and RESPIR's own theorem statements draw for their PIR construction \cite{godinot2026respir} -- stating the covered adversary class precisely, rather than an unconditional claim, is the standard this literature already holds itself to.

A second, practical caveat: Proposition~\ref{prop:zerodiv} is exact for literal, single-string uniform suppression, but a server that adds cheap, content-preserving noise to an otherwise-canned reply (paraphrasing its own filler, injecting filler tokens) could in principle defeat a naive SimHash-based diversity check without restoring genuine informativeness. This specific arms race -- diversity-channel robustness against a server that knows it is being diversity-checked -- is out of scope here, which evaluates the concrete adversary it implements (uniform canned-reply suppression), not every conceivable output-filtering strategy.
...
Three conjectures organize an attack on this question, in increasing order of difficulty: (A) \emph{distribution-free achievability} -- a calibration-only test achieving power within $O(1/\sqrt m)$ of an oracle that knows the mechanism's noise distribution, via standard concentration inequalities; (B) \emph{a matching minimax lower bound} against the adaptive adversary above, via a Le Cam- or Fano-style argument distinguishing two \emph{server strategies} rather than two datasets -- the least well-trodden of the three; and (C) \emph{adversarial multi-query composition}, treating the audit as a repeated game between an auditor's falsification-response budget and a no-regret learner. (A) most likely reduces to a known concentration-inequality corollary once correctly stated -- still a real, if modest, contribution; (B)'s most interesting outcome is a clean impossibility result with a non-obvious dependence on $m$. This is realistically months of focused work in hypothesis testing or DP theory, offered as a scoped, falsifiable agenda, not a preview of its answer.
