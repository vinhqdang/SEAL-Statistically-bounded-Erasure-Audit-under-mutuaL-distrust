...
\centering
\footnotesize
\caption{Full SEAL-W certificate, final (Bonferroni-corrected) version, all four server behaviors (3 trials, $N_{\text{calib}}{=}2$, $\beta_{\text{target}}{=}0.10$; rates here carry the same wide 3-trial confidence intervals as Table~\ref{tab:llmprogression}). The last row is a theoretical prediction, not an empirical rate -- see prose.}
\label{tab:llmfull}
\begin{tabular}{@{}lc@{}}
\toprule
Server behavior & Detection rate (empirical) \\
\midrule
