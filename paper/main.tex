...
Table~\ref{tab:waterdrum} reports the result against a server that uniformly suppresses every query mentioning the forgotten entity's name (\texttt{topic\_filter}), at $n_{\text{trials}}=30$, $N_{\text{calib}}=6$. Every one of the 30 trials shows the real cluster's response diversity collapsing to \emph{exactly} zero (Proposition~\ref{prop:zerodiv} proves this must happen, not merely that it tends to), and the full SEAL-W certificate (diversity channel combined with the slot channel via the Bonferroni split) flags the attack 93.3\% of the time (CI $[0.779, 0.992]$), with the diversity channel alone at 86.7\% (CI $[0.693, 0.962]$). The WaterDrum-equivalent baseline, run on the exact same 30 fine-tuned models, flags it \emph{zero} times (CI $[0.000, 0.116]$) -- structurally unable to, since a single canonical query has no diversity signal to check in the first place. This is a large, statistically decisive gap: at a smaller calibration budget ($N_{\text{calib}}=2$), the same comparison gave only a narrow, inconclusive edge (10\% vs.\ 7\%, kept in git history rather than restated here), so the decisiveness reported here comes from the calibration-budget fix (Section~\ref{sec:discussion}), not from re-running the same experiment until it looked better. For completeness, the WaterDrum-equivalent baseline's own false-accusation rate (0.167, CI $[0.056, 0.347]$) is higher than SEAL-W's (0.067) at this same budget, since it has no analog of SEAL-W's exact finite-sample threshold correction (Theorem~\ref{thm:predquantile}). This is precisely the false negative WaterDrum's Appendix D reports as unsolved, and at this calibration budget SEAL-W's diversity channel closes it decisively.
...
This paper motivates itself from GDPR's Article 17 erasure right \cite{gdpr2016} and the CJEU's Google Spain judgment \cite{googlespain2014}, which established that a controller can be compelled to act against its own interest -- but neither settles what evidentiary weight a calibrated statistical certificate, with a stated but nonzero false-accusation rate $\beta$, would carry with a regulator or court. No jurisprudence says whether a $\beta{=}0.05$-controlled flag is proof, probable cause, or inadmissible; we take no position and state plainly that this is unresolved. Relatedly, running SEAL-W (real PIR, committed batches, replicated calibration) is not something an individual data subject will realistically do by hand -- in practice this is machinery for a regulator, advocacy body, or aggregator acting on subjects' behalf, per this paper's own threat model (Section~\ref{sec:threat}: ``a data owner, \emph{or a regulator acting on its behalf}''), not a tool each affected individual runs themselves.
